    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--bg-main)' }}>
            ...
            <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
                <h1 style={{ fontSize: '26px', color: 'var(--text-heading)', marginBottom: '32px' }}>Foydalanuvchi xabarlari</h1>
                ...
                <div className={selected ? "grid-2-col" : ""} style={{ display: 'grid', gap: '24px' }}>
                    <div>
                        {loading ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                {contacts.map(contact => (
                                    <div key={contact.id} onClick={() => handleRead(contact)} style={{ background: selected?.id === contact.id ? 'var(--gold-pale)' : 'var(--bg-card)', borderRadius: '12px', padding: '16px 20px', border: `1px solid ${selected?.id === contact.id ? 'var(--gold)' : 'var(--border-color)'}`, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.15s' }}>
                                        <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: contact.read ? 'transparent' : 'var(--gold)', flexShrink: 0, border: contact.read ? '1px solid var(--border-color)' : 'none' }} />
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                                                <span style={{ fontFamily: 'var(--font-display)', fontSize: '15px', color: 'var(--text-heading)', fontWeight: contact.read ? '400' : '600' }}>{contact.name}</span>
                                                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-muted)' }}>{formatDate(contact.createdAt as unknown as string)}</span>
                                            </div>
                                            <div style={{ fontSize: '13px', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{contact.subject}</div>
                                        </div>
                        ...
                        {totalPages > 1 && (
                            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '24px' }}>
                                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={{ padding: '8px 16px', border: '1px solid var(--border-color)', borderRadius: '8px', background: 'var(--bg-card)', cursor: page === 0 ? 'not-allowed' : 'pointer', color: page === 0 ? 'var(--text-muted)' : 'var(--text-heading)', fontFamily: 'var(--font-mono)', fontSize: '13px' }}>←</button>
                                {Array.from({ length: totalPages }, (_, i) => (
                                    <button key={i} onClick={() => setPage(i)} style={{ padding: '8px 14px', border: '1px solid', borderColor: page === i ? 'var(--gold)' : 'var(--border-color)', borderRadius: '8px', background: page === i ? 'var(--gold)' : 'var(--bg-card)', color: page === i ? '#061d15' : 'var(--text-heading)', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '13px' }}>{i + 1}</button>
                                ))}
                                <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1} style={{ padding: '8px 16px', border: '1px solid var(--border-color)', borderRadius: '8px', background: 'var(--bg-card)', cursor: page === totalPages - 1 ? 'not-allowed' : 'pointer', color: page === totalPages - 1 ? 'var(--text-muted)' : 'var(--text-heading)', fontFamily: 'var(--font-mono)', fontSize: '13px' }}>→</button>
                            </div>
                        )}
                    </div>

                    {selected && (
                        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '28px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)', position: 'sticky', top: '80px', alignSelf: 'start' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '18px', color: 'var(--text-heading)' }}>Xabar tafsiloti</h3>
                                <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', alignItems: 'center' }}><Icons.X /></button>
                            </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                {[
                                    ...
                                    <div key={i}>
                                        ...
                                        <div style={{ fontSize: '15px', color: 'var(--text-heading)' }}>{item.value}</div>
                                    </div>
                                ))}
                                <div>
                                    ...
                                    <div style={{ fontSize: '15px', color: 'var(--text-main)', lineHeight: '1.8', background: 'var(--bg-secondary)', padding: '16px', borderRadius: '8px', whiteSpace: 'pre-wrap' }}>{selected.message}</div>
                                </div>
