        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-main)' }}>
            ...
            <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Qidirish..." style={{ padding: '8px 16px', border: '1px solid var(--border-color)', borderRadius: '20px', fontSize: '14px', fontFamily: 'var(--font-body)', outline: 'none', minWidth: '200px', background: 'var(--bg-card)' }} />
                </div>
                ...
                {showForm && (
                    <div style={{ background: 'var(--bg-card)', borderRadius: '12px', padding: '32px', marginBottom: '32px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-md)' }}>
                        <h2 style={{ fontSize: '20px', color: 'var(--text-heading)', marginBottom: '28px' }}>{editItem ? 'Manbani tahrirlash' : 'Yangi manba qo\'shish'}</h2>
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-muted)' }}>Hali manbalar yo'q</div>
                ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {filtered.map(item => (
                            <div key={item.id} style={{ background: 'var(--bg-card)', borderRadius: '12px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '16px', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)', flexWrap: 'wrap' }}>
                                <div style={{ width: '56px', height: '72px', borderRadius: '6px', background: item.coverUrl ? 'transparent' : 'var(--bg-header)', flexShrink: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
