...
export default function AdminResourcesPage() {
    ...
    const fetchItems = async () => {
        setLoading(true)
        try { const data = await resourceService.getAll(0, 100); setItems(data.content) }
        catch { setError('Manbalar yuklanmadi') }
    ...
    const handleDelete = async (id: number) => {
        ...
        try { await resourceService.delete(id); setItems(prev => prev.filter(i => i.id !== id)) }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editItem) { const u = await resourceService.update(editItem.id, form); setItems(prev => prev.map(i => i.id === editItem.id ? u : i)) }
            else { const c = await resourceService.create(form); setItems(prev => [c, ...prev]) }
            handleCancel()
    ...
    const filtered = items
        ...
        .filter(i => search ? i.title.toLowerCase().includes(search.toLowerCase()) || i.author.toLowerCase().includes(search.toLowerCase()) : true)
        ...
        <div style={{ minHeight: '100vh', background: 'var(--off-white)' }}>
            ...
            <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Qidirish..." style={{ padding: '8px 16px', border: '1px solid rgba(27,58,107,0.2)', borderRadius: '20px', fontSize: '14px', fontFamily: 'var(--font-body)', outline: 'none', minWidth: '200px', background: '#fff' }} />
                </div>
                ...
                {showForm && (
                    <div style={{ background: '#fff', borderRadius: '16px', padding: '32px', marginBottom: '32px', border: '1px solid rgba(27,58,107,0.1)', boxShadow: 'var(--shadow-md)' }}>
                        <h2 style={{ fontSize: '20px', marginBottom: '24px', color: 'var(--navy-dark)' }}>{editItem ? 'Manbani tahrirlash' : 'Yangi manba'}</h2>
                        <form onSubmit={handleSubmit}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
                                <div><label style={labelStyle}>Sarlavha *</label><input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} required style={inputStyle} placeholder="Manba nomi" /></div>
                                ...
                                <div>
                                    ...
                                    <FileUpload folder="resources/covers" accept="image/*" label="Muqova yuklash" onUpload={(url) => setForm(p => ({ ...p, coverUrl: url }))} />
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {filtered.map(item => (
                            <div key={item.id} style={{ background: '#fff', borderRadius: '12px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '16px', border: '1px solid rgba(27,58,107,0.08)', boxShadow: 'var(--shadow-sm)' }}>
                                <div style={{ width: '56px', height: '72px', borderRadius: '6px', background: item.coverUrl ? 'transparent' : 'linear-gradient(135deg, var(--navy-dark), var(--navy))', flexShrink: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    {item.coverUrl ? <img src={item.coverUrl} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ color: 'rgba(255,255,255,0.4)' }}>{getIconLg(item.resourceType)}</div>}
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
                                        <h3 style={{ fontSize: '16px', color: 'var(--navy-dark)', fontFamily: 'var(--font-display)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.title}</h3>
                                        <span style={{ padding: '2px 10px', borderRadius: '20px', fontSize: '11px', fontFamily: 'var(--font-mono)', background: 'rgba(201,168,76,0.12)', color: 'var(--gold)', whiteSpace: 'nowrap' }}>{item.resourceType}</span>
                                    </div>
                                    <div style={{ fontSize: '13px', color: 'var(--gray-600)' }}>{item.author}{item.publishedYear ? ` · ${item.publishedYear}` : ''}{item.pageCount ? ` · ${item.pageCount} sahifa` : ''}</div>
                                </div>
