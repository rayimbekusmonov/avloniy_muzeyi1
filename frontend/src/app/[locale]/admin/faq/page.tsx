...
export default function AdminFaqPage() {
    ...
    const handleAutoTranslate = async () => {
        if (!form.questionUz.trim() && !form.answerUz.trim()) {
            setError("Avtomatik tarjima qilish uchun kamida O'zbekcha savol yoki javob kiritilishi kerak")
            return
        }
        setTranslating(true)
        setError('')
        ...
        try {
            const res = await translateBatch({
                question: form.questionUz,
                answer: form.answerUz,
            }, ['ru', 'en'])

            setForm(prev => ({
                ...prev,
                questionRu: res.ru.question || prev.questionRu,
                questionEn: res.en.question || prev.questionEn,
                answerRu: res.ru.answer || prev.answerRu,
                answerEn: res.en.answer || prev.answerEn,
            }))

            setTranslateSuccess("Savol va javob Rus hamda Ingliz tillariga muvaffaqiyatli tarjima qilindi! Iltimos, tillar bo'yicha ko'zdan kechirib chiqing.")
                ...
                {showForm && (
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '32px', boxShadow: 'var(--shadow-md)', marginBottom: '40px' }}>
                        ...
                        <form onSubmit={handleSubmit}>
                            {/* Language tab & Auto-Translate button */}
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '20px' }}>
                                <div style={{ display: 'flex', gap: '8px', background: 'var(--bg-secondary)', padding: '6px', borderRadius: '8px', width: 'fit-content' }}>
                                    {LANGS.map(l => (
                                        <button
                                            key={l.key}
                                            type="button"
                            ...
                            {translateSuccess && (
                                <div style={{ background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.35)', borderRadius: '8px', padding: '12px 16px', marginBottom: '20px', color: '#22c55e', fontSize: '13.5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span>✓</span>
