    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--bg-main)' }}>
            ...
            <main style={{ maxWidth: '1000px', margin: '0 auto', padding: '48px 24px' }}>
                <h1 style={{ fontSize: '28px', marginBottom: '8px', color: 'var(--text-heading)' }}>Boshqaruv paneli</h1>
                <p style={{ color: 'var(--text-muted)', marginBottom: '40px' }}>Muzey kontentini boshqaring</p>
                    ...
                    {menuItems.map((item, i) => (
                        <Link key={i} href={item.href} style={{ textDecoration: 'none' }}>
                            <div className="card" style={{ padding: '32px' }}>
                                ...
                                <h3 style={{ fontSize: '18px', color: 'var(--text-heading)', marginBottom: '8px' }}>{item.title}</h3>
                                <p style={{ fontSize: '14px', color: 'var(--text-muted)' }}>{item.desc}</p>
                                <div style={{ marginTop: '20px', fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--gold)', fontWeight: '700' }}>Boshqarish →</div>
                            </div>
                        </Link>
                    ))}
                </div>
